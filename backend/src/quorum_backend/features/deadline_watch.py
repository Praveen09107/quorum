...
- tasks: the real, committed effort hours due before a user's
  ...
  before that same deadline (`retry_queue_drainer.py`'s own
  `available_hours_before_deadline`, also reused directly).
...
no queue row to lock, only the user being processed). A user who
already has a real, unresolved negotiation (`negotiations.resolved_at
IS NULL`) is skipped, never given a second, duplicate one for the
same still-open conflict.
"""
...
async def _has_unresolved_negotiation(conn: asyncpg.Connection, *, user_id: str) -> bool:
    row = await conn.fetchrow(
        "SELECT 1 FROM negotiations WHERE user_id = $1 AND resolved_at IS NULL LIMIT 1",
        uuid.UUID(user_id),
    )
...
async def scan_one_user(conn: asyncpg.Connection, *, user_id: str) -> tuple[ScanOutcome, str | None]:
    ...
    available_hours = available_hours_before_deadline(deadline)
        ...
        try:
            async with pool.acquire() as conn:
                async with conn.transaction():
                    await conn.fetchrow("SELECT user_id FROM users WHERE user_id = $1 FOR UPDATE", uuid.UUID(user_id))
