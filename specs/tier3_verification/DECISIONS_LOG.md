...
**Verified live:** `ruff check backend` clean. `pytest backend/tests -q` → **400 passed, 5 failed** — all 5 real, live, unmocked, external Gemini-quota `429`s, none a code defect: 3 pre-existing (`test_negotiation_gemini_calls.py`, `DEC-121`), 2 new (`test_downstream_translation.py`'s own live tests), all hitting the identical, still-fluctuating quota `DEC-126` already corrected the record on this same session's earlier entry. The real, live-database `test_retry_queue_drainer.py` suite (17 tests, including a genuinely live proof that `deadline_conflict_check` reads real, already-committed hours from the real `tasks` table and correctly forces a real Stage-A-only `revise`) and the real route-level `test_main.py` additions (4 tests, including one genuinely live end-to-end call reaching the real drainer) all pass.
