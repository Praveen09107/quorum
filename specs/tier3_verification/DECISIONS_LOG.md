...
**Real idempotency, not just a plausible-looking check:** a real `SELECT ... FOR UPDATE` on the user's own `users` row, held for the duration of that user's check-and-create transaction, serializes a concurrent second invocation (a real possibility once `pg_cron` fires against a `--max-instances=2` deployment) — a different real primitive from `retry_queue_drainer.py`'s own `FOR UPDATE SKIP LOCKED`, since there is no queue row to lock here, only the user being processed. A user who already has a real, unresolved negotiation is skipped, never given a duplicate.

**A real bug found and fixed by this session's own adversarial self-review, before any review subagent ran:** an earlier version of `run_deadline_watch()`'s per-user loop had no exception isolation — a genuine database error for one real user would propagate out and abort the scan for every other real user too, and since this route runs on a periodic schedule (not retried within the same request, unlike `retry_queue_drainer.py`'s own per-job retry queue), one bad user could silently block every real user's negotiation trigger until the next scheduled run — directly contradicting this module's own docstring, which already (incorrectly) claimed this isolation existed. Fixed: each real user's own check-and-create is now wrapped in its own `try`/`except`, logged via `logger.exception()`, tallied into a new `DeadlineWatchResult.users_failed` field, and the loop always continues. A real, live regression test (`test_run_deadline_watch_a_real_failure_for_one_user_never_blocks_the_rest`, forcing a genuine `uuid.UUID()` failure via a malformed user_id) proves the fix.

**A real, disclosed test-safety boundary, found and designed around before writing a single test:** `run_deadline_watch()` iterates the ENTIRE real `users` table by design — including this deployment's one real, live, non-test account (`scripts/seed_demo_dataset.py`'s own established fact). An automated test calling it unscoped would risk a real, unwanted write against real, production-meaningful data as a side effect of running the test suite. Fixed by design, not by avoidance: `run_deadline_watch()` gained an explicit `user_ids: list[str] | None = None` parameter — `None` (the real, live default `main.py` uses) scans the whole real table; `test_deadline_watch.py` always passes an explicit, real, test-owned list instead. The route-level test in `test_main.py` proves the real wiring via a monkeypatched `run_deadline_watch()`, the same "prove the wiring, not re-prove the underlying logic" precedent already established there for `/internal/drain-retry-queue`'s own 200 test — never a real, unscoped call either.

**Verified live:** `ruff check backend` clean. Every file this session touched or added — `test_deadline_watch.py` (7 tests), the relevant subset of `test_retry_queue_drainer.py` (21 tests, confirming the public rename didn't break anything), and the new deadline-watch tests in `test_main.py` (4 tests) — passes cleanly and repeatedly in isolation. A genuinely clean full-suite run (422 passed, 5 failed — all the already-disclosed, fluctuating Gemini-quota pattern, `STATUS_INDEX.md` item #21) was captured before this session's own self-review fix; two later full-suite attempts hit real, external local-network DNS flakiness (`getaddrinfo failed` against both the Gemini API host and the Supabase database host from this specific machine's current network — confirmed via direct `nslookup`/isolated single-test reruns, not a code regression), disclosed here rather than silently retried into a misleadingly clean number.

**NOT YET CALLED BY A REAL SCHEDULE:** `pg_cron`/`pg_net` remain confirmed, live, NOT enabled on the real Supabase project (`DEC-127`, unresolved since). `backend/scripts/enable_deadline_watch_cron.sql` (new) has the real, ready-to-run SQL (a 30-minute cadence, deliberately less frequent than `drain-retry-queue`'s 5 minutes — this scans slow-moving real data, not urgent enqueued work) — genuinely unverified live until Preethish enables both extensions.

**Genuinely still open, not resolved by this entry:** the other three Phase 2 jobs (briefing, follow-up, spend-alert); real Gemini-backed detail generation for an autonomously-created negotiation; `pg_cron`/`pg_net` enablement (Preethish's own action); the CRITICAL-tier cross-model review this Rule-6 session still needs before merge; the end-of-phase whole-system checkpoint (sign in on a real device, let a real cron interval pass, confirm a real negotiation appears in Today's "In motion" without anyone running a script) — blocked on `pg_cron` enablement, tracked, not silently skipped.
