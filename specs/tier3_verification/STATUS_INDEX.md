...
| `router.py`, `agents/*`, `negotiation/*`, `auth/*`, `security/*`, `features/*` | Not yet built in this repository. |
...
| **Backend total, this repository** | **62/62 real, passing tests** (`ruff check backend` clean). `pytest backend/tests -q` — verified live this session. |
...
3. `router.py` (`get_stakes()`, `compute_complexity()`) — specified, not yet built. The Gate's core decision-making (all 9 validators + orchestration) is now real; the Router is the genuinely next session.
