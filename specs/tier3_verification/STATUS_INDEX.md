...
| **Backend total, this repository** | **28/28 real, passing tests** (`ruff check backend` clean). `pytest backend/tests -q` — verified live this session. |
...
3. Remaining backend validators (`TemporalFactCheck`, `BudgetCheck`, `PIILeakCheck`, `ProvenanceCheck`, `CoverageCheck`) — specified, not yet built in this repository.
