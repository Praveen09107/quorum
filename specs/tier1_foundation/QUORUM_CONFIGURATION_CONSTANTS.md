...
|---|---|---|
| Full | ≥ 8GB | Primary — **Llama 3.2 3B, resolved live by Sprint 0 for real, on a real physical Android device, run to genuine completion via a real, complete head-to-head comparison (`DECISIONS_LOG.md` `DEC-130`/`DEC-131`).** Real, mechanical decision (`ModelBenchmark.decideWinner()`'s own logic, per `IMPL_00`'s design): both real Full-tier candidates genuinely downloaded, loaded, and ran real, live on-device inference against all 6 of Sprint 0's real test prompts — **Gemma 4 E4B: 17% validity (1/6 passed), 0.1 tok/s, 19–41s per prompt; Llama 3.2 3B: 67% validity (4/6 passed), 0.8 tok/s, 3–13s per prompt** (`DEC-131`). The validity gap (50 points) is far beyond `decideWinner()`'s 5-point closeness threshold, so **Llama 3.2 3B wins decisively on real, measured accuracy and speed** — not merely because Gemma failed to download, which is what an earlier real attempt in this same session showed before the harness's retry logic was corrected (a real, ordinary mid-transfer network interruption on a ~4.7GB file, reaching 74% before exhausting a too-small retry budget — a transfer-length/retry-budget mismatch, precisely diagnosed and fixed in `DEC-130`, not a device/network/DNS defect). `DEC-111`'s original "emulator DNS relay failure" theory is superseded entirely. Per this project's own "trust the mechanical decision, never second-guess it with human intuition after the fact" discipline, this stands as the real, current, decided outcome. |
