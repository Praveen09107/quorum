...
**✅ INGESTION HALF CLOSED, `DEC-140`–`DEC-141`; EXECUTION HALF NOT STARTED.** Real Gmail polling (`features/email_ingestion.py`), a real `sent_messages` table (migration `0011`), and `features/waiting_on.py` are built, CRITICAL-tier reviewed (1 BLOCKER + 3 HIGH + 4 MEDIUM found and fixed — the BLOCKER was reachable on the single most common real email pattern and was enshrined by this PR's own original test), deployed (revision `quorum-backend-00015-bpq`), and live-verified end to end: an automated test sends a real email through Gmail's real API and confirms it's genuinely detected; a real, live smoke test against production recorded 5 real sent messages on first run and correctly 0 on a repeat (idempotency); the real `pg_cron` job (`email-ingestion`, jobid `8`) is scheduled and its first real, unattended fire was directly confirmed (`net._http_response.status_code = 200`). `GET /waiting_on` and `POST /internal/email-ingestion` are wired into `main.py`; `fetchWaitingOn` is wired into `main.dart`, reaching the You tab's already-built Waiting On screen for the first time. A real, live infrastructure discovery mid-session: this backend's `SUPABASE_URL` connects through Supabase's PgBouncer pooler in transaction-pooling mode, which does not support session-scoped Postgres advisory locks reliably — the job's own overlap guard uses a singleton lock *row* instead (migration `0012`), not an advisory lock. Deliberately split, per this section's own original recommendation below: `SEND_EMAIL`/`ARCHIVE_EMAIL`/`LABEL_EMAIL` execution and Career's own Email-riding detection are NOT started, tracked as this phase's real remaining scope. Full detail: `DECISIONS_LOG.md` `DEC-140`–`DEC-141`.
