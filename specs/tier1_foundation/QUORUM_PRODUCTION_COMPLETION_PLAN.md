...
**✅ Gate Reveal CLOSED, `DEC-146`** — real `findings`/`objections` persistence directly on `action_events` (migration `0013`, two new nullable JSONB columns, no companion table needed) plus a real `GET /gate_reveal/{proposal_id}`. CRITICAL-tier reviewed (touches `retry_queue_drainer.py`, the Gate's own verdict-persistence pipeline). A real, disclosed limit: `objections` will always be a real, honest empty list through this one real production path today, since none of the three real negotiation-eligible domains this drainer translates is ever real `Stakes.S3` (the only tier that produces real Critic objections) — proven correct via a dedicated, direct unit test of the persistence function itself, not just the full pipeline. Full detail: `DECISIONS_LOG.md` `DEC-146`.
