...
**🔶 IN PROGRESS, `DEC-132`/`DEC-133`.** `deadline-watch` and `spend-alert` (2 of the 4 named jobs) are real, tested, and built; briefing and follow-up remain open, a real, disclosed scope narrowing made explicit rather than assumed. A real, structural correction found while building deadline-watch: Career is not a first-class negotiation domain in the real schema (`Position.domain`/`ResourceClaim.claim_type` only ever resolve to `calendar`/`tasks`/`finance`) — see `DEC-132`. spend-alert's own real trigger condition (a newly-detected recurring subscription's cost vs. remaining budget, alongside overcommitted tasks) was confirmed directly with Preethish before building it, per `QUORUM_ARCHITECTURE_DESIGN_DOCUMENT.md` §8.6's own "spontaneous-spend-vs-known-upcoming-cost" framing — see `DEC-133`, which also factored the two jobs' shared logic into `features/negotiation_trigger_support.py` and added a real, precise `trigger_source` column (migration `0008`) so the two jobs' own detections never suppress each other. Full detail: `STATUS_INDEX.md` item #30, `DECISIONS_LOG.md` `DEC-132`/`DEC-133`.
