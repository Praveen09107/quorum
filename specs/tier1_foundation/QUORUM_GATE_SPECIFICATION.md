...
**Authoritative source:** `backend/gate/prompts.py`, tested (`backend/tests/test_gate_prompts.py`, 9/9 passing).

**A real, disclosed correction (`DEC-125`):** this section previously claimed `CRITIC_SYSTEM_PROMPT`/`JUDGE_SYSTEM_PROMPT` already existed as "full text, real," with "4/4 passing" tests, naming a specific test that did not exist anywhere in this repository. Confirmed directly, before any code was written this session: `prompts.py`'s own top-of-file docstring said the opposite — "deliberately not built yet" — and `test_gate_prompts.py` had exactly 2 tests, neither about the Critic or Judge. This section was simply wrong about this repository's real state; corrected here, and the real prompts built for real, per `DEC-125`.

### 5.1 Model assignment (restated from `QUORUM_MASTER_REFERENCE.md`, load-bearing enough to repeat here)

Generator: Gemini Flash. Critic: ~~Groq-hosted Llama 3.3 70B~~ **`openai/gpt-oss-120b`, Groq-hosted — a real, disclosed, live-forced substitution (`DEC-125`): a real, live call to `llama-3.3-70b-versatile` returned a 404, and a real, live `GET /openai/v1/models` against this project's own Groq key confirmed the current catalog has no Llama chat model left at any size.** Judge: Gemini Flash (`gemini-3.6-flash`, the same real model `negotiation/gemini_calls.py` already discovered live at `DEC-121`). **The load-bearing property this section actually protects — a genuinely different model PROVIDER for the Critic than the Generator/Judge, deliberate model diversity — is preserved**; only the specific historical model name is gone from Groq's own catalog. The Critic's model diversity is not a cost decision; it is the same reasoning the project's own AI-assisted development methodology (§15 of the ADD, `CLAUDE.md` Rule 6) applies to *building* Quorum — same-family review shares blind spots.

### 5.2 The Critic prompt (full text, real, in `prompts.py::CRITIC_SYSTEM_PROMPT`, `DEC-125`)

Key structural properties, each independently tested:
- **Obligation to critique, enforced by instruction, backed by schema:** the prompt states at least one real objection or an explicit signed-off entry is required; `Objection.signed_off` exists precisely so a genuine "nothing wrong" is distinguishable from a lazy empty response. Belt-and-suspenders in code too: `gate/llm_calls.py::make_groq_critic_call` substitutes a real, code-assigned signed-off fallback if the model ever returns zero objections anyway, never trusting the instruction alone.
...
### 5.3 The Judge prompt (full text, real, in `prompts.py::JUDGE_SYSTEM_PROMPT`, `DEC-125`)

- **Anonymization is an orchestration-layer responsibility, not a prompt-layer one** — the prompt's docstring states this explicitly: role-stripping and objection-order-randomization happen in the calling code *before* this function runs, so that discipline can be independently unit-tested rather than trusted to prompt phrasing alone. Real as of `DEC-125`: `gate/anonymization.py::randomize_objection_order()`, called from `gate/llm_calls.py::make_gemini_judge_call` immediately before this prompt is built. Role-stripping needed no separate code: `Objection` has no identity/author field for any code to strip in the first place, confirmed by direct reading before this was built.
