...
- §5 (REST): §5.1 verified live. §5.2–§5.16 are all specified, not implemented as live HTTP endpoints — none were ever wired into a real FastAPI route and executed end to end, since no live deployment exists. This is unchanged from the original state of this section; what changed is only that this line previously stopped at "§5.2–5.4" and silently never mentioned the twelve endpoint sections (§5.5–§5.16) added across later sessions. **Important distinction, not to be lost in the "not implemented" label:** most of these endpoints wrap business logic that *is* real and tested at the function level — `waiting_on.py`, `search.py`, `trust_digest.py`, and others are genuinely real; what's specified-but-unbuilt is only the thin REST layer connecting each to an HTTP route. §2's schema index names exactly which real module backs each one.
