...
**RESOLVED, `DEC-127`:** "re-entering the Gate at its own stakes level" is now literal, not aspirational — `features/retry_queue_drainer.py` (new) reads real, due `retry_queue` rows, translates each domain in a chosen option's `source_domains` into a real `ActionProposal` (`negotiation/downstream_translation.py`, new, real Gemini-backed), and calls the real `gate.review()` (`DEC-125`'s real Critic/Judge) for each, persisting a real `action_events` row per downstream action. Exposed via a new, real `POST /internal/drain-retry-queue`, gated by a real, static shared secret (`X-Internal-Secret`) rather than a user Bearer token, since `pg_net` calling it has no real user session. **A real, disclosed, honest scope boundary that remains, not closed by this entry:** the drainer produces a real Gate *verdict* per downstream action and stops there — it does not execute the approved action's real-world effect (no real `INSERT INTO tasks`/`expenses`, no real Calendar call); no code anywhere in this backend has ever executed a Gate-approved proposal's real effect, for any action, confirmed by direct search before this session. A second real, disclosed gap: `pg_cron`/`pg_net`, the mechanism meant to call this endpoint on a real schedule, are confirmed live NOT enabled on the real Supabase project as of this session — `backend/scripts/enable_retry_queue_drain_cron.sql` has the real, ready-to-run SQL, genuinely unverified until Preethish enables both extensions via the Supabase dashboard.
...
Response (`200`), a `calendar` result naming a real external invitee -- genuinely reviewed, genuinely never executed:
...
```json
{
  ...
  "findings": [ { "...": "a real Finding, see §1.3" } ],
  "objections": [ { "...": "a real, genuine Critic objection, see §1.4 -- Stage B's full debate DOES run for this one real case" } ]
}
```
...
One real, unified extraction call classifies a real user's own free text into exactly one of five real domains today, `"tasks"`, `"finance"`, `"calendar"`, `"career"`, or `"email"` (`QUORUM_FINAL_COMPLETION_PLAN.md` Session 7, `DEC-173` — the fifth and final real domain) — `domain` is always present; every field belonging to the *other* domains is always present too, but always `null`. `title` is `tasks`-only; `amount`/`category`/`payee`/`finance_action` are the `finance`-domain equivalent; `event_start`/`event_end`/`event_title`/`calendar_action` are the `calendar`-domain equivalent; `company`/`new_status` are the `career`-domain equivalent; `email_recipient`/`email_action` are the `email`-domain equivalent. `finance_action`/`calendar_action`/`email_action` are the real, resolved `FinanceAction`/`ActionType` values (`"log_expense"`/`"update_budget"`/`"update_expense"`/`"delete_expense"`, `"create_calendar_event_local"`/`"create_calendar_event_external"`, `"send_email"`) — never the free-form text a person typed.
...
`stakes` is `"S1"` for `CREATE_TASK`/`UPDATE_TASK`/`LOG_EXPENSE` (RESOLVED, a real, disclosed follow-up review finding, `DEC-172` F-D, corrected here: `UPDATE_APPLICATION_STATUS` is NOT in this group -- see the next clause), `"S2"` for `UPDATE_BUDGET`/`CREATE_CALENDAR_EVENT_LOCAL`/`DELETE_TASK`/`UPDATE_EXPENSE`/`DELETE_EXPENSE`/`UPDATE_APPLICATION_STATUS`, or `"S3"` for `CREATE_CALENDAR_EVENT_EXTERNAL`/`SEND_EMAIL` (the Judge only runs for `S2`; `gate.orchestration.run_stage_b()` invokes the Critic only for `S3` -- the two real cases on this route where the FULL Stage B debate genuinely runs, both the real Groq Critic and the real Gemini Judge). This extraction call stays on Gemini rather than Groq for all five domains specifically because `CLAUDE.md`'s architecture fact groups the real Generator and Judge together as one same-provider unit; for `calendar`/`email` specifically, this is not just a provider-grouping technicality -- a Groq-backed extraction call would put the real Critic in the position of reviewing a proposal drafted by that same Groq model, for real, since the Critic genuinely runs for `S3`. See `features/quick_capture.py`'s own top-of-file docstring for the full account.

**Resolving WHICH existing real record an update/delete refers to is done entirely in code, never a second real LLM call** (`QUORUM_FINAL_COMPLETION_PLAN.md` Session 6's own single most safety-critical property, reused directly for Session 7's own real recipient resolution) — the model returns only a short, free-text `reference_description` (or, for `email`, `recipient_description`); the backend matches it deterministically against the real, currently-addressable candidates for that domain, scoped to the authenticated user, and fails loud (a real `502`) on zero or multiple genuine matches, never guessing. **Calendar editing/cancellation is deliberately NOT supported** — a real local calendar event has no real, addressable server-side row to resolve a reference against or execute a change on (real local-event ground truth belongs on-device); see `features/quick_capture.py`'s own top-of-file docstring for the full, disclosed scope correction.

**A real, disclosed, safety-driven fact, not an implementation gap:** a genuine `calendar`/`email` result with `executed == false` and `decision == "approve"` is the ORDINARY, expected outcome for `CREATE_CALENDAR_EVENT_EXTERNAL`/`SEND_EMAIL` today, not a rare exception like it is for `finance`/`tasks`/`career`. `CREATE_CALENDAR_EVENT_LOCAL` has no real execution target anywhere in this backend (real local-event ground truth belongs on-device); `CREATE_CALENDAR_EVENT_EXTERNAL`/`SEND_EMAIL`'s real `Stakes.S3` human-approval backstop refuses to auto-execute a real Google Calendar booking, or a real Gmail send, from this route's own Gate verdict alone, matching `CLAUDE.md`'s absolute rule that S3 actions always require a separate, explicit human approval -- no such real approval endpoint exists anywhere in this backend today (see `action_executor.py`'s own top-of-file docstring). This route can genuinely review a calendar or email request end to end and correctly explain why, but it can never book, create, or send one.

**`email` domain (Session 7):** `recipient_description` (a real, short phrase naming WHO, resolved against this user's own real, prior `sent_messages` -- `features/waiting_on.py`'s own real table -- entirely in code) and `recipient_email` (a literal, real email address, ONLY if genuinely written in the text, mirroring `invitee_email`'s own established rule exactly) are the real request-side inputs; `email_recipient` (populated only when genuinely executed -- never through this route today) and `email_action` (`"send_email"`, populated regardless of `executed`, matching `calendar_action`'s own established convention) are the real response-side fields. **A real, disclosed, deliberate scope boundary:** Quorum can only resolve a recipient this user has genuinely emailed before through their own real Gmail account -- a bare first name for someone never emailed has no real candidate to resolve to, and correctly, honestly fails loud (a real `502`) rather than guessing at a plausible-looking address.
...
- §5 (REST): §5.1 and §5.15 (`GET /trust_digest`, Batch 10 Phase 3 Part B, `DEC-100`) are both verified live on the actual deployed Cloud Run service — a real, authenticated request against the real public URL confirms `/trust_digest` genuinely round-trips through the real database in production, not just through a local `TestClient`. §5.2–§5.14 and §5.16 remain specified, not implemented as live HTTP endpoints at all — none of those were ever wired into a real FastAPI route and executed end to end. **Important distinction, not to be lost in the "not implemented" label:** most of these endpoints wrap business logic that *is* real and tested at the function level — `waiting_on.py`, `search.py`, `self_test_harness.py`, and others are genuinely real; what's specified-but-unbuilt is only the thin REST layer connecting each to an HTTP route. §2's schema index names exactly which real module backs each one.
