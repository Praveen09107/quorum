...
**A real finding from actually cross-checking, not assumed away:** the same three patterns and test strings were run through Python's `re` (already proven correct for the backend side) before writing the Dart tests around them. That check found a genuine, real overlap — a 16-digit credit card number's first 12 digits also satisfy the Aadhaar-style pattern, with a real word boundary landing exactly where the pattern needs one. This is disclosed and tested explicitly, not hidden: `scan()` genuinely reports both categories for a pure card number; `redact()` correctly produces only one redaction, because patterns apply sequentially and credit-card consumes those digits first. Both behaviors are proven by test, not assumed from reading the regex.
