...
## Whole-system verification checkpoint — mandatory, added after a real, confirmed drift incident
**Real, found, disclosed fact this rule exists to prevent from recurring:** a full repository diagnosis found that 9 consecutive real sessions (`DEC-120` through `DEC-128`) — Search, negotiation content, negotiation-choose, Gate Stage B, the `retry_queue` drainer, real execution — were each genuinely rigorous *within their own backend scope* (real database, real tests, real CI, real review) but **none of them checked whether the mobile app, or the product as a whole, had actually moved forward.** The result: real, tested backend capability accumulated for months while the actual running app drifted further from being a usable, autonomous product — a negotiation subgraph with zero real production callers, five real mobile screens (including a permanent bottom-nav tab) with no backend to connect to, zero write/create UI anywhere. Every individual session's own verification passed. The system, as a whole, was not being checked. This is the real mechanism of the drift Preethish found — not a single wrong technical decision, a missing checkpoint.

**The rule:** passing a session's own backend tests is necessary, never sufficient, for calling that session's work integrated. At the end of every phase of `specs/tier1_foundation/QUORUM_PRODUCTION_COMPLETION_PLAN.md` (or whatever multi-session plan is currently active), and at minimum every 2–3 sessions during any other stretch of real implementation work, run a **whole-system checkpoint** before starting the next phase or session:
1. `ruff check backend` + the real backend test suite, `flutter analyze` + `flutter test` — all real, all clean.
2. A real, live hit against the actual deployed Cloud Run backend for whatever the recent work touched — not a local-only check.
3. Whenever a real device or working emulator is available: an actual on-device run, checking specifically whether a real, signed-in user can now see or do something they couldn't before.
4. Update the **"Product Reality" section** of `STATUS_INDEX.md` (see below) honestly, even if the honest answer is "nothing user-visible changed this stretch, by design."

This checkpoint is a real, required gate — a phase or a multi-session stretch does not get called complete without it, the same standing every other verification step in Rule 2 already has.

**`STATUS_INDEX.md`'s "Product Reality" section:** a short, living list, rewritten (not appended to) at every checkpoint above, tracking a genuinely different axis than the rest of that file. `STATUS_INDEX.md`'s existing content answers "is this code real and tested" — it has done that well throughout this project's history. This new section answers a different, previously-unasked question: **"what can a real, signed-in user currently see and do, end to end, right now"** — every real, reachable screen and tab, every real interaction, every real autonomous behavior actually observed firing on its own. Anything not on this list, however real and tested the underlying code is, is not yet real to a user. This is the section that would have caught the drift this rule exists to prevent, had it existed sooner.
